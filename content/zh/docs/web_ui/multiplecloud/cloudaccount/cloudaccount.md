...
3. 配置以下参数：
   ...
   - 同步策略：通过设置标签与资源映射项目的对应关系，将资源按照规则设置分配给指定项目，同步策略仅在云账号同步的时候生效，默认关闭。
   - 同步策略生效范围：打开同步策略后显示，资源标签根据资源标签匹配并修改资源归属项目，项目标签根据项目标签匹配并修改项目映射关系，从而修改资源归属项目。
...
3. 配置以下参数：
    ...
    - 账号、密码：即登录Proxmox控制台的账号和密码。在安装好的ProxmoxVE系统中，会有一组内建的预设帐号root，这个帐号就是Linux本机权限系统的一员。Linux的帐号认证系统为PAM，因此选择纳管PAM认证方式的账号应在账号后缀加上@pam，例如root@pam。除了采用PAM认证机制外，ProxmoxVE管理平台层也有自己的帐号系统，因此选择纳管PVE认证方式的账号应在账号后缀加上@pve，例如admin@pve。
    - 域：选择云账号所属的域。当云账号私有状态下，域下所有项目用户都可以使用云账号创建资源。
    - 资源归属方式：默认指定项目，还支持根据云上项目归属。
    - 资源归属项目：资源归属方式选择指定项目时，可以选择将云账号上的资源同步到{{<oem_name>}}平台的本地项目，本地项目会根据域来过滤。资源归属方式选择根据云上项目归属时，资源会同步与云上项目同名的本地项目中，若资源无云上项目属性则归属到指定项目中。
    - 同步策略：通过设置标签与资源映射项目的对应关系，将资源按照规则设置分配给指定项目，同步策略仅在云账号同步的时候生效，默认关闭。
