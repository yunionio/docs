...
- 主机加密：启用主机加密，主机的所有磁盘将被所选的密钥加密。仅{{<oem_name>}}平台支持。
- 自定义数据：通过自定义数据来配置弹性云服务器，支持通过文本或上传json文件的方式。{{<oem_name>}}平台支持shell、python、perl的脚本，私有云和公有云平台支持shell、yaml的脚本。
- 堡垒机：堡垒机开关默认关闭，打开后支持选择{{<oem_name>}}平台纳管的堡垒机。选择堡垒机后，需设置堡垒机资源节点、端口、特权用户、普通用户，设置完成后，虚拟机在创建完成后会自动加入到该堡垒机中。
...
#### 添加到堡垒机
支持将虚拟机添加到平台已纳管的堡垒机中，目前仅支持JumpServer v3版本。

1. 在虚拟机页面，单击虚拟机右侧操作列 **_"更多"_** 按钮，选择下拉菜单 **_"属性设置-添加到堡垒机"_** 菜单项，弹出添加到堡垒机对话框。
2. 选择堡垒机后，配置以下参数：
   - 堡垒机：选择平台纳管的堡垒机。
   - 堡垒机资源节点：选择堡垒机的资源节点。
   - 端口：堡垒机主机地址的端口。
