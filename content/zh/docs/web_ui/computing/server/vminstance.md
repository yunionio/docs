...
- 是否配置GPU：是否为虚拟机配置GPU卡，启用该项后，还需要选择GPU类型和具体的GPU卡，设置图形GPU后，镜像会过滤出UEFI。
...
| 绑定GPU卡      | 支持             | 不支持       | 不支持        | 不支持      | 不支持      | 不支持        | 不支持   | 不支持 | 不支持     | 不支持        | 不支持            | 不支持       | 不支持      | 不支持      | 不支持      | 不支持      | 不支持        | 不支持      | 不支持      | 不支持      | 不支持      | 不支持 | 不支持 |
...
3. 查看以下信息。
    ...
    - 配置信息：包括操作系统、IP地址、MAC地址、系统镜像、宿主机、安全组、VPC、CPU、内存、系统盘、数据盘、ISO、GPU、Daemon虚拟机、最大带宽、监控地址等；
    - 加密：加密密钥、密钥算法、密钥持有者等；
