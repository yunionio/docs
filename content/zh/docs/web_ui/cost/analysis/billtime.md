...
title: "时间"
weight: 6
description: 以时间的维度展示每个月的费用情况。
---

以时间的维度展示每个月的费用情况。

包年包月分摊开关：当开启包年包月分摊后，平台会将包年包月的资源费用分摊到每天或每月。时间页面的消费金额将变为按量付费的费用+包年包月分摊到每天或每月的费用。如按计费模式查看时，可以看到按量付费和包年包月分摊。包年包月分摊开关属于全局开关，开启关闭后影响整个费用菜单。
...
该功能用于查看指定月份的当月消费、当年消费、消费趋势以及不同维度下的消费占比。

1. 在时间页面，单击指定月份名称项，进入指定月份下实际成本页面。
...
3. 查看以下信息：
    ...
    - 查看每个月不同平台、资源类型、资源、云账号、云订阅、区月、计费模式、项目的消费金额和占总消费数的比例。
