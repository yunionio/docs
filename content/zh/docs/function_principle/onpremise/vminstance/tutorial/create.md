...
- 只有{{<oem_name>}}平台虚拟机支持最多添加8个网络，其他平台仅支持一个网络。
    {{% /alert %}}
        ...
        - 指定IP子网：选择VPC和IP子网，如需指定静态IP，可单击 **_"手动配置IP"_** 按钮，设置IP地址，如需指定MAC地址，可可单击 **_"手动配置MAC地址"_** 按钮，设置MAC地址。
...
1. 设置完成后，用户可单击 **_新建_** 按钮，创建虚拟机。
2. 若启用了工单，则需要填入申请原因，单击 **_"提交工单"_** 按钮，提交创建虚拟机的工单，等待审批通过后才会开始创建虚拟机。
