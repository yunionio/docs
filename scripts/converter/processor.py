...
META_KEY_OEM_IGNORE = 'oem_ignore'
...
class DirProcess(object):
    ...
    def include_by_oem(self, oem_mode):
        def f(fp, meta):
            return _include_by_oem(fp, meta, oem_mode)
        return self.append_func(f)
...
def _include_by_oem(filepath, meta, oem_mode):
    oem_ignore = meta.get(META_KEY_OEM_IGNORE, False)
    if oem_ignore and oem_mode:
        print("%s: ignored in oem_mode" % filepath)
        return False
    return True
